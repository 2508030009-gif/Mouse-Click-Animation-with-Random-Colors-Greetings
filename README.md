# Mouse-Click-Animation-with-Random-Colors-Greetings
A simple interactive web project that creates animated circles on every mouse click. Each circle appears at the click position with a random color and greeting text in different languages. The circles expand and fade out using CSS animations, creating a fun visual click effect built using HTML, CSS, and JavaScript.
Here’s a **complete, polished, viral-style `README.md`** you can directly copy into your repo 👇


## ⚙️ How It Works

1. User clicks anywhere on the page
2. JavaScript captures click coordinates (`clientX`, `clientY`)
3. A new `div` is created dynamically
4. Random:
   - Color 🎨
   - Greeting 🌍
5. Element is positioned at click location
6. CSS animation runs (scale + fade-out)
7. Element is removed after 5 seconds 🧹

---

## 🌍 Random Greetings Included

This project includes greetings like:

* Hi
* Namaste 🙏
* Bonjour 🇫🇷
* Hola 🇪🇸
* Ciao 🇮🇹
* こんにちは (Japanese)
* 안녕하세요 (Korean)
* مرحبًا (Arabic)
* नमस्ते (Hindi)
* * many more…

---

## 🤯 Why this project is cool

This project demonstrates:

* DOM manipulation skills
* Event handling in JavaScript
* CSS animations & keyframes
* UI micro-interactions
* Creative front-end thinking

---

## 👨‍💻 Author

Made with ❤️ by **Anmol Dikshit**

---

## 📜 License

This project is open-source and free to use under the MIT License.
